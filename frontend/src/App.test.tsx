...
function decision(overrides: Partial<components["schemas"]["DecisionResponse"]> = {}) {
  return {
    ...
    ranked_carriers: [],
    comparable_loads: [{ load_id: "hist-1", external_id: "FF-1001", carrier_rate_usd: "1180.00" }],
    warnings: [],
    ...overrides,
...
describe("App", () => {
  it("shows fictional-broker selector and active-load summary", async () => {
    ...
    const selector = await screen.findByLabelText("Fictional broker");
    await waitFor(() => {
  ...
  it("switches tenant, stores only its ID, and invalidates load queries", async () => {
    ...
    const selector = screen.getByLabelText("Fictional broker") as HTMLSelectElement;
  ...
  it("shows a high-confidence decision with canonical timing and comparable evidence", async () => {
    ...
    expect(screen.getByText("$1,250.00 expected carrier rate")).toBeInTheDocument();
    expect(screen.getByText("$1,100.00–$1,300.00 historical comparison range")).toBeInTheDocument();
    expect(screen.getByText("High confidence")).toBeInTheDocument();
    expect(screen.getByText("Near exact lane evidence")).toBeInTheDocument();
    expect(screen.getByText("6 historical loads · 5.3 effective")).toBeInTheDocument();
    expect(screen.getByText("FF-1001 · $1,180.00")).toBeInTheDocument();
  ...
  it("shows medium and low confidence with explicit evidence warnings", async () => {
    const lowDecision = decision({
      pricing: {
        ...
        warnings: ["Sparse historical evidence."],
      },
      confidence: { level: "LOW", score: "0.31", components: {} },
      warnings: ["Target equipment is unknown."],
    });
    vi.stubGlobal(
      "fetch",
      vi.fn((url: string) =>
    ...
    expect(screen.getByText("Regional lane blended with metro corridor")).toBeInTheDocument();
    expect(screen.getByText("Sparse historical evidence.")).toBeInTheDocument();
    expect(screen.getByText("Target equipment is unknown.")).toBeInTheDocument();
  });
  ...
  it("renders carrier rankings in rank order with structured evidence details", async () => {
    ...
    await userEvent.setup().click(screen.getAllByText("Evidence loads (1)")[0]);
    expect(screen.getByText("load-version-1")).toBeInTheDocument();
  });
  ...
  it("makes sparse and missing-deadhead evidence explicit without operational claims", async () => {
    ...
    expect(
      await screen.findByText("Limited history pulls this score toward the neutral prior."),
    ).toBeInTheDocument();
    expect(
      screen.getByText("No valid historical delivery-to-pickup distance is available."),
    ).toBeInTheDocument();
